...
lang = emptyDef {identStart=letter <|> char '_',
                 ...
                 reservedNames=["int", "float", "for", "if"]}
...
data Expr = Var String | LitI Integer | LitF Double | Add Expr Expr | Sub Expr Expr
                  | Mul Expr Expr | Div Expr Expr | Inc Expr
                  | Deref Expr | Index Expr Expr | Assgn Expr Expr
                  | Lt Expr Expr | Gt Expr Expr | Call String [Expr] deriving (Show)
data DecoratedVarName = Smpl String | Pointer String | Array String Integer deriving (Show)
...
p_for = do
  ...
  body <- p_stmts <|> (:[]) <$> p_stmt
  return $ For init cond rept body

p_if = keyword "if" *> (If <$> paren (p_expr) <*> (p_stmts <|> (:[]) <$> p_stmt))

p_deco_name = op "*" *> (Pointer <$> ident) 
            <|> try (Array <$> ident <*> bracket(number))
            <|> Smpl <$> ident

p_var_decl = VarDecl <$> p_type_smpl <*> (p_deco_name `sepBy` op ",")
            <* op ";"


p_return = keyword "return" *> (Return <$> p_expr <* op ";")
p_empty_stmt = do op ";" ; return Mpty

p_stmt = p_for <|> p_if <|> p_var_decl 
               <|> p_return <|> p_empty_stmt <|> Expr <$> p_expr <* op ";"
